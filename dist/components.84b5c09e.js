class Spollers {
    constructor(wrapClass, multiOpen = false){
        ...
        this.events();
        if (this.activeSpollers) this.activeSpollers.forEach((spoller)=>{
    ...
    events() {
        this.spollers.forEach((spoller)=>{
            spoller.addEventListener('click', (event)=>{
                ...
                if (spoller.open) this.closeSpoller(spoller);
                else this.openSpoller(spoller);
            });
        });
    }
    openSpoller(spoller) {
        ...
        if (this.activeSpoller && this.activeSpoller !== spoller && !this.multiOpen) this.closeSpoller(this.activeSpoller);
...
//# sourceMappingURL=components.84b5c09e.js.map
